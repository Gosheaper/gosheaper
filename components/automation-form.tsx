...
export default function AutomationForm() {
  ...
  const [validationMessage, setValidationMessage] = useState<{ type: 'success' | 'error' | null; message: string }>({ type: null, message: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!webhookUrl) {
      setValidationMessage({ type: 'error', message: 'Error: Webhook URL not configured' });
    ...
    try {
      ...
      if (res.ok) {
        setValidationMessage({ type: 'success', message: 'Form submitted successfully! We will contact you shortly.' });
        setFormData({
          name: '',
          email: '',
          automationType: '',
        ...
        setValidationMessage({ type: 'error', message: 'An error occurred during submission. Please try again.' });
      }
    } catch (error) {
      setValidationMessage({ type: 'error', message: 'Unable to contact server. Please check your internet connection.' });
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl border border-gray-100 p-8 md:p-12">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              ...
              className={`w-full font-semibold text-lg py-4 px-6 rounded-lg transition-all duration-200 transform ${
                isSubmitting
                  ? 'bg-gray-400 text-gray-200 cursor-not-allowed'
                  : 'bg-blue-600 text-white hover:bg-blue-700 focus:ring-4 focus:ring-blue-200 hover:scale-[1.02]'
              }`}
            >
